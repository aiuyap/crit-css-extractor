...
export async function extractCriticalCSS(options: CriticalCSSOptions): Promise<string> {
  // implementation
}
...
export async function extractCSS(url: string): Promise<string> {
  try {
    ...
    throw new CriticalExtractionError(`Failed to extract CSS for ${url}`, error)
  } finally {
    await page?.close?.()
  }
}
